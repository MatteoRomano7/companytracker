// =============================================================================
// FMP Financial Ratios API Route — Proxy for financial ratios data
// GET /api/fmp/ratios/{symbol}?period={period}&limit={limit}
// =============================================================================

import { NextRequest, NextResponse } from 'next/server';
import { getFinancialRatios, FMPError, ApiSchemaError, NetworkError, RateLimitedError } from '@/services/api';
...
export async function GET(
  ...
  try {
    ...
    // Call the service
    const ratios = await getFinancialRatios(symbol.toUpperCase(), period, limit);

    return NextResponse.json(ratios, { status: 200 });
  } catch (error) {
    ...
    console.error('[FMP Financial Ratios Route] Unexpected error:', error);
    return NextResponse.json(
